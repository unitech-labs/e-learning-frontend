...
2. Dialog hiện lên với:
   ...
   - Nút "Thêm học sinh"
3. Teacher nhập email và bấm "Thêm học sinh này"
4. Backend tự động:
...
- Thêm học sinh vào course và classroom
- Generate account mới nếu email chưa có trong hệ thống:
  - Account email format: `{random_string}@generated.local` (ví dụ: `EVEVIRYF3948@generated.local`)
  - `email_linked`: Email mà teacher nhập vào (ví dụ: `example@gmail.com`)
- Enroll học sinh vào classroom
- Gửi email thông tin đăng nhập đến `email_linked` (email mà teacher nhập vào, nếu account mới được tạo)
...
```json
{
  ...
  "user": {
    ...
    "email_linked": "example@gmail.com",
    "first_name": "",
    "last_name": "",
    "full_name": "",
...
1. **Quick enroll - User đã có account:**
...
2. **Quick enroll - User chưa có account:**
   - Request: `POST /api/v1/classrooms/{classroom_id}/quick-enroll/` với email mới
   - Expected: Tạo account mới với `account_type: 'generated'`, tạo order, enroll, gửi email, `is_new_account: true`

3. **Quick enroll - Email đã enrolled:**
...
- Account được generate tự động sẽ có:
  ...
  - `email`: Format `{random_string}@generated.local` (ví dụ: `EVEVIRYF3948@generated.local`) - email của account trong hệ thống
...
- Khi teacher nhập email (ví dụ: `example@gmail.com`), hệ thống sẽ:
  1. Tạo order với status "confirmed/paid"
  2. Thêm học sinh vào course và classroom
  3. Generate account mới (nếu email chưa có) với `email_linked = example@gmail.com`
