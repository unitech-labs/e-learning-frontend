...
// Get hardware_signature from localStorage (returned by BE on login)
function getDeviceId(): string {
  if (typeof window !== 'undefined' && typeof localStorage !== 'undefined') {
    return localStorage.getItem('hardware_signature') || ''
  }
  return ''
}
...
function createHeaders(url: string, token: string | null, customHeaders: Record<string, string> = {}) {
  ...
  // For login/google endpoints: only send X-Device-Type (device_profile goes in body)
  // For other auth endpoints: send both X-Device-ID and X-Device-Type
