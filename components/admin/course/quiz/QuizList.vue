...
const quizzesByChapter = computed(() => {
  ...
  quizzes.value.forEach(quiz => {
    if (!grouped[quiz.chapter]) {
      grouped[quiz.chapter] = {
        chapter_title: quiz.chapter_title,
        quizzes: []
      }
    }
    grouped[quiz.chapter].quizzes.push(quiz)
  })
...
const fetchQuizzes = async () => {
  try {
    ...
    if (props.courseId) {
      // If courseId is provided, we might need to filter by course
  ...
  <div class="flex flex-col gap-6 w-full">
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          ...
          <div class="flex items-center gap-4 text-sm">
            <div class="flex items-center gap-2 px-3 py-2 bg-blue-50 rounded-lg">
              ...
              <span class="font-medium text-blue-900">{{ quizzes.length }} {{ $t('quiz.management.totalQuizzes') }}</span>
            </div>
            <div class="flex items-center gap-2 px-3 py-2 bg-green-50 rounded-lg">
              <Icon name="tabler:eye" class="text-green-600" />
              <span class="font-medium text-green-900">
                {{ quizzes.filter(q => q.is_published).length }} {{ $t('quiz.management.published') }}
              </span>
            </div>
            <div class="flex items-center gap-2 px-3 py-2 bg-orange-50 rounded-lg">
              ...
              <span class="font-medium text-orange-900">
                {{ quizzes.filter(q => !q.is_published).length }} {{ $t('quiz.management.draft') }}
              </span>
            </div>
          </div>
    ...
    <div v-else-if="error" class="bg-red-50 border border-red-200 rounded-lg p-4">
      ...
      <a-button 
        type="link"
        size="small"
        class="mt-2 text-red-600 hover:text-red-800 p-0 h-auto"
        @click="fetchQuizzes"
      >
    ...
    <div v-else-if="quizzes.length === 0" class="text-center py-8">
      ...
      <a-button 
        type="link"
        size="small"
        class="mt-2 text-green-600 hover:text-green-800 p-0 h-auto"
        @click="handleCreateQuiz"
      >
    ...
    <div v-else class="flex flex-col gap-6">
      <div
        v-for="chapter in quizzesByChapter"
        :key="chapter.chapter_title"
        class="flex flex-col gap-4"
      >
        <!-- Chapter Title -->
        <h3 class="text-lg font-semibold text-slate-900 border-b border-gray-200 pb-2">
          ...
          >
            ...
            <div class="flex items-center gap-2">
              ...
              <a-dropdown>
                <template #overlay>
                  <a-menu @click="({ key }: { key: string }) => handleTogglePublished(quiz.id, quiz.is_published)">
                    <a-menu-item key="toggle">
                      <div class="flex items-center gap-2">
                        <Icon 
                          :name="quiz.is_published ? 'tabler:eye-off' : 'tabler:eye'" 
                          class="text-sm" 
                        />
                        <span>{{ quiz.is_published ? $t('quiz.management.unpublish') : $t('quiz.management.publish') }}</span>
                      </div>
                    </a-menu-item>
                  </a-menu>
                </template>
                <a-button
                  size="small"
                  class="!flex items-center gap-2 !h-8 border-gray-300 text-sm font-medium"
                ...
                >
                  <Icon 
                    :name="quiz.is_published ? 'tabler:eye' : 'tabler:eye-off'" 
                    class="text-sm" 
                  />
              ...
              <a-button
                size="small"
                class="!flex items-center gap-2 !h-8 text-red-600 border-red-300 hover:bg-red-50"
    ...
    <a-modal
      ...
      :title="$t('quiz.management.deleteConfirm.title')"
      :width="480"
      centered
