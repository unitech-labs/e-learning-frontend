...
function handleEditComment(comment: QuestionComment) {
  editingComment.value = comment
  showCommentModal.value = true
}

async function handleDeleteComment(comment: QuestionComment) {
  try {
...
function handleCommentSuccess(_comment: QuestionComment) {
  emit('refresh')
}
  ...
  <div class="border-t border-gray-200 pt-4">
    ...
    <div v-else class="text-center py-6 text-gray-500">
      ...
      <p class="text-xs text-gray-400 mt-1">
        Hãy thêm nhận xét để phản hồi cho học sinh
      </p>
    </div>
