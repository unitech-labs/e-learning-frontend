...
export function useDateUtils(initTexts, EnUs) {
  ...
  const addDatePrototypes = () => {
    if (!texts.value.today)
      texts.value = EnUs // If no texts, use EnUs.
  ...
  const isSameDate = (date1, date2) => {
    if (!date1 || !date2)
      return console.warn(`Vue Cal: missing date${!date1 ? '1' : '2'} parameter for comparison with \`isSameDate(date1, date2)\`.`)
    else if (!isValid(date1))
      return console.warn(`Vue Cal: invalid date1 provided for comparison with \`isSameDate(date1, date2)\`: \`${date1}\`.`)
    else if (!isValid(date2))
      return console.warn(`Vue Cal: invalid date2 provided for comparison with \`isSameDate(date1, date2)\`: \`${date2}\`.`)
  ...
  const isInRange = (date, rangeStart, rangeEnd) => {
    if (!isValid(date))
      return console.warn(`Vue Cal: invalid date provided to \`isInRange(date, rangeStart, rangeEnd)\`: \`${date}\`.`)
  ...
  const formatDate = (date, format = 'YYYY-MM-DD', txts = null) => {
    if (!txts)
      txts = texts.value
    if (!format)
      format = 'YYYY-MM-DD' // Allows passing null for default format.
    if (format === 'YYYY-MM-DD')
      return formatDateLite(date)
    ...
    return format.replace(/(\{[a-z]+\}|[a-z]+)/gi, (m, contents) => {
  ...
  const formatTime = (date, format = 'HH:mm', txts = null, round = false) => {
    ...
    const formatted = format.replace(/(\{[a-z]+\}|[a-z]+)/gi, (m, contents) => {
