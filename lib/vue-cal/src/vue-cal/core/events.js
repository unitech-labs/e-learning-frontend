 ...
 */
export function useEvents(vuecal) {
  ...
  const events = computed(() => {
    ...
    for (const event of sortedEvents) {
      ...
      // Remove 1ms in case the event ends at next midnight 00:00:00.
        ...
        if (!events.byYear[year])
          events.byYear[year] = {}
        if (!events.byYear[year][month])
          events.byYear[year][month] = {}
  ...
  const normalizeEventDates = (event) => {
    ...
    if (isNaN(event.start) || isNaN(event.end) || (event.end.getTime() < event.start.getTime())) {
      if (isNaN(event.start))
        console.error(`Vue Cal: invalid start date for event "${event.title}".`, event.start)
      else if (isNaN(event.end))
        console.error(`Vue Cal: invalid end date for event "${event.title}".`, event.end)
  ...
  const injectMetaData = (event) => {
    ...
    if (event._.deleting === undefined)
      event._.deleting = false
    if (event._.deleted === undefined)
   ...
   * @param {number} [forcedStage] - The stage of deletion to force.
  ...
  const deleteEvent = async (eventIdOrCriteria, forcedStage = 0) => {
    if (!eventIdOrCriteria)
      return console.warn('Vue Cal: Cannot delete event without its ID or criteria.')
    ...
    if (index === -1)
      return console.warn(`Vue Cal: Cannot delete unknown event \`${eventId}\`.`)

    const event = config.events[index]
    if (event.deletable === false)
      return console.warn(`Vue Cal: Can't delete event \`${eventId}\` since it was explicitely set to \`delete: false\`.`)
  ...
  const getCellOverlappingEvents = (cellStart, cellEnd, allDay) => {
    ...
    if (!cellEvents.length)
      return { cellOverlaps: {}, longestStreak: 0 }
  ...
  const getEventsInRange = (start, end, { excludeIds = [], schedule = null, background = true, allDay = false } = {}) => {
    ...
    if (!Object.keys(events.value.byId).length)
      return []
    ...
    if (Object.keys(events.value.byId).length <= 100) {
      for (const event of Object.values(events.value.byId)) {
        if (!event || excludeSet.has(event._.id))
          continue
        if (schedule !== null && schedule !== event.schedule)
          continue
        if (background === false && event.background)
          continue
      ...
      for (let month = monthFrom; month <= monthTo; month++) {
        ...
        for (const dayStr in days) {
          ...
          for (let i = 0; i < dayEventIds.length; i++) {
            ...
            if (!e || excludeSet.has(e._.id))
              continue
            if (schedule !== null && schedule !== e.schedule)
              continue
            if (background === false && e.background)
              continue
            if (config.allDayEvents && ((allDay && !e.allDay) || (!allDay && e.allDay)))
              continue
            // Accept events that overlap the range.
  ...
  const computeEventStartEnd = (event, cellStart) => {
    ...
    if (resizeState.moveX && vuecal.touch?.currentHoveredCell && resizeState.cellEl) {
      // Get the current hovered cell date from data attribute.
      const hoveredCellStart = vuecal.touch.currentHoveredCell?.dataset?.cellStart
      const currentCellDate = hoveredCellStart ? new Date(Number(hoveredCellStart)) : null
  ...
  const onDocumentMousemove = async (e) => {
    ...
    if (resizeState.fromResizer && resizeState.resizingEvent) {
      // Get the cell start date from the data attribute
      const cellStartTs = resizeState.cellEl?.dataset?.cellStart
      if (!cellStartTs)
        return
      ...
      if (acceptResize !== false) {
        ...
        if (resizeEventHandler)
          resizeState.resizingLastAcceptedEvent = { ...resizeState.resizingEvent, _: { ...resizeState.resizingEvent._ } }
      }
    }
  }
