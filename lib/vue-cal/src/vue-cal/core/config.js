...
export function useConfig(vuecal, props, attrs) {
  const { dateUtils } = vuecal
  const ready = false
  const view = computed(() => {
    if (availableViews.value[props.view])
      return props.view

    const fallbackView = props.datePicker ? 'month' : 'week'
    const view = props.view || fallbackView
    if (availableViews.value[view])
      return view
  ...
  const eventListeners = computed(() => {
    ...
    for (const [attr, value] of Object.entries(attrs)) {
      ...
      if (m0)
        listeners[m1.toLowerCase()][kebabize(m2).replace(/^-+|-+$/g, '')] = value
    }
  ...
  const hideWeekdays = computed(() => {
    ...
    if (props.hideWeekends)
      (weekDays[6] = true) && (weekDays[7] = true)
  ...
  const availableViews = computed(() => {
    ...
    else if (props.horizontal && !views) {
      return {
  ...
  const selectedDate = computed(() => {
    if (typeof props.selectedDate === 'string')
      return dateUtils.stringToDate(props.selectedDate)
    if (props.selectedDate instanceof Date)
      return props.selectedDate
    if (!props.selectedDate)
      console.log('Vue Cal: Info - The provided selected date is undefined.')
    ...
    if (Array.isArray(props.disableDays)) {
      for (let date of props.disableDays) {
        ...
        if (typeof date === 'string')
          jsDate = dateUtils.stringToDate(date)
      ...
      console.warn('Vue Cal: The provided `disableDays` prop is invalid:', props.disableDays)
    }
  ...
  const minTimestamp = computed(() => {
    ...
    else if (props.minDate && props.minDate instanceof Date)
      date = props.minDate
  ...
  const maxTimestamp = computed(() => {
    ...
    else if (props.maxDate && props.maxDate instanceof Date)
      date = props.maxDate
    return date?.getTime() || null
  })
  ...
  if (props.locale || !vuecal.texts.today)
    loadTexts(props.locale || 'en-us')
