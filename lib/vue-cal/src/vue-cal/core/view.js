...
export function useView({ config, dateUtils, emit, texts, eventsManager }, vuecalEl) {
  ...
  const end = computed(() => {
    if (viewId.value === 'month')
      return new Date(startTheoretical.value.getFullYear(), startTheoretical.value.getMonth() + 1, 0, 23, 59, 59, 999)
  ...
  const cols = computed(() => {
    ...
    if (!config.availableViews[viewId.value])
      return 1
  ...
  const firstCellDate = computed(() => {
    if (viewId.value === 'month') {
      ...
      if (config.startWeekOnSunday && !config.hideWeekdays[7])
        weekday += 1
      if (config.viewDayOffset)
        weekday -= config.viewDayOffset
    ...
    if (viewId.value === 'week') {
      ...
      if (config.startWeekOnSunday && !config.hideWeekdays[7])
        firstVisibleDay = 1
      if (config.viewDayOffset)
        firstVisibleDay += config.viewDayOffset
  ...
  function getMonthName(monthIndex, monthsArray, shouldTruncate = false) {
    if (!monthsArray || !monthsArray[monthIndex])
      return monthIndex + 1 // Numeric fallback.
  ...
  function switchView(id, emitUpdate = true, date = null) {
    ...
    if (availableViews.includes(id)) {
      ...
      if (emitUpdate && viewId.value !== id)
        emit('update:view', id) // Emit the view change only if the view is actually changing.
      viewId.value = id
      if (date)
      ...
      !!console.warn(`Vue Cal: the \`${id}\` view is not available.`)
    }
  }

  function switchToBroaderView() {
    if (broaderView.value)
      switchView(broaderView.value)
    else console.warn('Vue Cal: no broader view is available.')
  }

  function switchToNarrowerView() {
    if (narrowerView.value)
      switchView(narrowerView.value)
  ...
  function navigate(forward = true) {
    ...
    switch (viewId.value) {
      ...
      case 'days':
        ...
        if (forward)
          newViewDate = dateUtils.addDays(lastCellDate.value, 1)
      ...
      case 'week': {
        if (forward) {
          ...
          newViewDate = dateUtils.subtractDays(extendedStart.value, cellsCount.value)
        }
        break
      }
      case 'month': {
  ...
  function updateViewDate(date, emitUpdate = true, forceUpdate = false) {
    if (!dateUtils.isValid(date))
      return console.warn('Vue Cal: can\'t navigate to the given date: invalid date provided to `updateViewDate(date)`.')
    ...
    if (viewId.value === 'month')
      ([viewStart, viewEnd] = [start.value, end.value])
  ...
  watch(() => cellsCount.value, () => {
    if (cellsCount.value > 90)
      console.warn('Vue Cal: high cell count detected. Performance may degrade when interactions are enabled.')
  })
  watch(() => config.watchRealTime, (watchRealTime) => {
    if (watchRealTime && config.time)
      initTimeTicker()
