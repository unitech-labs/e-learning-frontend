  ...
  switch (view.id) {
    ...
    case 'days':
      if (config.availableViews.days.rows > 1)
        dateUtils.formatDate(props.start, 'D')
      return ''
    case 'week':
      return ''
    case 'month':
...
const eventStyles = computed(() => {
  if (view.isMonth || view.isYear || view.isYears || props.allDay)
    return {}
  ...
  for (const event of cellEvents.value) {
    ...
    if (config.stackEvents) {
      ...
      styles[eventId][widthOrHeight] = `${100 / maxConcurrent}%`
    }
  }
  return styles
})
...
const specialHours = computed(() => {
  ...
  if (!Array.isArray(daySpecialHours))
    daySpecialHours = [daySpecialHours]
  ...
  return daySpecialHours.map((dayRanges) => {
    ...
    if (isNaN(from) || isNaN(to) || config.timeFrom >= to || config.timeTo <= from)
      return
...
const isDisabled = computed(() => {
  ...
  if (disableDays.length && disableDays.includes(dateUtils.formatDate(props.start)) && !isYearsOrYearView)
    return true
...
const nowLine = reactive({
  show: computed(() => {
    if (!view.isDay && !view.isDays && !view.isWeek)
      return
    if (!isToday.value || !config.time || props.allDay)
      return
    if (config.timeFrom > dateUtils.dateToMinutes(view.now))
      return
    if (dateUtils.dateToMinutes(view.now) > config.timeTo)
      return
    return true
  }),
...
const cellEventListeners = computed(() => {
  if (isDisabled.value)
    return {} // If the cell is disabled, completely disable any interaction.
  ...
  for (const [eventListener, handler] of Object.entries(eventListeners)) {
    eventListeners[eventListener] = (e) => {
      ...
      if ((e.target || e.e?.target).closest?.('.vuecal__event'))
        return
...
function onCellClick() {
  ...
  if (config.clickToNavigate) {
    if ((view.isMonth || view.isDays || view.isWeek) && config.availableViews.day)
      view.switch('day')
    else if (view.isYear && config.availableViews.month)
      view.switch('month')
    else if (view.isYears && config.availableViews.year)
      view.switch('year')
  }
...
function onMousedown(e) {
  ...
  if (isTouchEvent) {
    ...
    touch.touchAndDragTimer = setTimeout(() => {
      ...
      if (touch.holding || touch.dragging)
        e.preventDefault()
    }, 500)
  }
...
async function createEventIfAllowed(e) {
  ...
  if (typeof createListener === 'function') {
    ...
    if (eventToCreate && typeof eventToCreate === 'object')
      view.createEvent(eventToCreate)
    if (eventToCreate && typeof eventToCreate === 'boolean')
      view.createEvent(eventCopy)
  }
  else {
    view.createEvent(eventToCreate)
  }
...
<template lang="pug">
.vuecal__cell(ref="cellEl" :class="classes" :data-cell-start="props.start.getTime()" v-on="cellEventListeners")
