...
const changeViewTimeout = null
const pressPrevOrNextInterval = null
...
export function useDragAndDrop(vuecal) {
  ...
  const cellDragEnter = (e, cell) => {
    ...
    if (e.currentTarget.contains(e.relatedTarget))
      return
    if (target === dragOverCell.el || !target.className.includes('vuecal__cell-content'))
      return false
  ...
  const cellDragOver = (e, cell) => {
    ...
    if (schedule || schedule === 0)
      cell.highlightedSchedule = schedule
  }
  ...
  const cellDragDrop = async (e, cell, allDay = false) => {
    ...
    if (incomingEvent.end)
      incomingEvent.end = new Date(incomingEvent.end) // Convert the string to Date.
