import { y as $, f as a, b as C, a as E, t as f, r as g, J as H, d as h, u as I, h as l, q as L, j as P, w as s, F as S, e as t, x as V, g as w, A as W } from './index-CgZSobh0.js'; import { c as U, s as z } from './index-CyRf_8z7.js'; import { _ as M } from './index-DCXHaOxe.js'

const F = { class: 'ml1 text-bold', href: 'https://https://vuejs.org/guide/essentials/event-handling.html', target: '_blank' }; const O = { class: 'logs-box bd1 bdrs2 ovh' }; const Y = { class: 'w-flex wrap align-center justify-end ml2 mr1' }; const J = { class: 'example mt6 mb2 mxa' }; const G = { class: 'example mt2 mb2 mxa' }; const q = { class: 'caption text-center' }; const K = { class: 'mxa my3', style: { 'max-width': '500px' } }; const Q = { class: 'w-flex gap2 basis-zero' }; const X = { class: 'w-flex gap2 my2 basis-zero' }; const Z = { class: 'code' }; const _ = { class: 'code' }; const ee = { class: 'code' }; const te = { class: 'w-flex align-center justify-center wrap gap2' }; const ae = { __name: 'dom-events', setup(ne) {
  const b = I(); const i = g('week'); g(null), g(null); const A = [{ start: '2018-10-30 10:30', end: '2018-10-30 11:30', title: 'Doctor appointment', content: '<i class="w-icon mdi mdi-hospital-box-outline"></i>', class: 'health', schedule: 1 }, { start: '2018-11-16 10:30', end: '2018-11-16 11:30', title: 'Doctor appointment', content: '<i class="w-icon mdi mdi-hospital-box-outline"></i>', class: 'health', schedule: 1 }, { start: '2018-11-19 10:35', end: '2018-11-19 11:30', title: 'Doctor appointment', content: '<i class="w-icon mdi mdi-hospital-box-outline"></i>', class: 'health', schedule: 1 }, { start: '2018-11-19 18:30', end: '2018-11-19 19:15', title: 'Dentist appointment', content: '<i class="w-icon mdi mdi-hospital-box-outline"></i>', class: 'health', schedule: 2 }, { start: '2018-11-20 18:30', end: '2018-11-20 20:30', title: 'Cross-fit', content: '<i class="w-icon mdi mdi-dumbbell"></i>', class: 'sport', schedule: 2 }, { start: '2018-11-21 11:00', end: '2018-11-21 13:00', title: 'Brunch with Jane', content: '<i class="w-icon mdi mdi-coffee-outline"></i>', class: 'leisure', schedule: 1, background: !1 }, { start: '2018-11-21 19:30', end: '2018-11-21 23:00', title: 'Swimming lesson', content: '<i class="w-icon mdi mdi-pool"></i>', class: 'sport', schedule: 2 }, { start: '2018-11-23 12:30', end: '2018-11-23 13:00', title: 'Macca\'s with Mark', content: '<i class="w-icon mdi mdi-food"></i>', class: 'leisure', schedule: 2 }, { start: '2018-11-23 21:00', end: '2018-11-23 23:30', title: 'Movie time', content: '<i class="w-icon mdi mdi-ticket"></i>', class: 'leisure', schedule: 1 }, { start: '2018-11-30 21:00', end: '2018-11-30 23:30', title: 'Another movie tonight', content: '<i class="w-icon mdi mdi-ticket"></i>', class: 'leisure', schedule: 1 }]; const N = g(null); const o = V({ expandedEmittedEvents: g(Array.from({ length: 30 }).fill(!1)), logs: g([]), clearEventsLog: () => o.logs = [], logMouseEvents: g(!1), logEvents: (r, e) => {
    let c; if (!o.logMouseEvents && r.includes('-mouse'))
      return !0; e.cell && (e.cell = { ...e.cell, events: e.cell.events.value }), e.e && (e.e = `[${e.e.constructor.name}]`), o.logs.push({ name: r, args: JSON.stringify(e, null, 2) }); const d = (c = N.value) == null ? void 0 : c.$el; return H(() => { let y; return (y = d == null ? void 0 : d.scrollTo) == null ? void 0 : y.call(d, { top: d.scrollHeight, behavior: 'smooth' }) }), !0
  } }); const m = V({ loading: !1, events: [], eventsTotal: g(0), onReady: ({ view: { start: r, end: e } }) => { m.events.push({ title: 'NAVIGATE WEEKS TO LOAD EVENTS!', start: new Date(r.addDays(3).setHours(10, 0, 0, 0)), end: new Date(r.addDays(3).setHours(14, 0, 0, 0)) }) }, onViewChange: async (r) => { m.loading = !0, await j(r.start.format(), r.end.format()), m.loading = !1 } }); const D = V({ selectedDate: g(null), viewDate: g(null) }); const p = V({ selectedDate: g(null), viewDate: g(null) }); const j = async (r, e) => { await new Promise(y => setTimeout(y, 400)); const d = z(r); const c = z(e); m.events = B(d, c), m.eventsTotal += m.events.length }; const B = (r, e) => {
    const d = U(r, e); const c = []; let y = 0; for (let v = 0; v < d; v++) {
      for (let T = 0; T < 10; T++) { const k = new Date(r.addDays(v).setHours(Math.floor(Math.random() * 8) + 9, Math.floor(Math.random() * 60), 0, 0)); const R = k.addHours(1); c.push({ title: `Event ${++y}`, start: k, end: R }) }
    } return c
  }; return (r, e) => {
    const d = E('w-icon'); const c = E('alert'); const y = E('router-link'); const v = E('w-button'); const T = E('ssh-pre'); const k = E('example'); const R = E('w-progress'); return h(), C(S, null, [a(k, { title: 'Vue Cal Emitted Events', anchor: 'emitted-events' }, { 'desc': s(() => [e[53] || (e[53] = t('p', { class: 'mb0' }, [l('Vue Cal emits events that you can listen to, to trigger an action outside of Vue Cal.'), t('br')], -1)), a(c, null, { default: s(() => [e[45] || (e[45] = l('If you are not familiar with Vue JS events, you should read about it in the', -1)), t('a', F, [e[44] || (e[44] = l('official Vue documentation.', -1)), a(d, { class: 'ml1', color: 'primary' }, { default: s(() => [...e[43] || (e[43] = [l('mdi mdi-open-in-new', -1)])]), _: 1 })])]), _: 1 }), t('p', null, [e[47] || (e[47] = l(`Here is a quick list of all the events that Vue Cal emits, but you can find more details in the
...
`, -1)])]), 'default': s(() => [t('div', G, [a(w(M), { 'dark': w(b).darkMode, 'time-from': 540, 'time-to': 1080, 'views-bar': !1, 'events': m.events, 'onReady': m.onReady, 'onViewChange': m.onViewChange, 'style': { height: '421px' } }, null, 8, ['dark', 'events', 'onReady', 'onViewChange']), m.loading ? (h(), W(R, { key: 0, class: 'mt-1' })) : L('', !0), t('div', q, [m.loading ? (h(), C(S, { key: 0 }, [l('Loading events...')], 64)) : (h(), C(S, { key: 1 }, [l(`${f(m.eventsTotal)} events loaded.`, 1)], 64))])])]), '_': 1 }), a(k, { title: 'External Controls & use of Vue Cal Methods', anchor: 'external-controls' }, { 'desc': s(() => {
      let n, x; return [e[79] || (e[79] = t('p', null, [l('You can access any '), t('strong', null, 'Vue Cal'), l(' internal method through Vue template ref.'), t('br'), l(`
